# mem_scanner.py - 进程内存特征搜索 (Hex/字符串)，LLM 模式输出上下文
# 移植自 pc-agent-loop。仅支持 Windows (ReadProcessMemory)。依赖: pip install yara-python
...
if sys.platform != "win32":
    def scan_memory(pid, pattern, context_size=256, mode="auto", llm_mode=False):
        return [f"Error: mem_scanner is Windows-only (kernel32.ReadProcessMemory). Current: {sys.platform}. On macOS/Linux use ptrace/proc or equivalent."]
else:
    import ctypes
    ...
    def build_rules(pattern, mode="auto"):
        try:
            ...
            return None, "yara not installed: pip install yara-python"
    ...
    def scan_memory(pid, pattern, context_size=256, mode="auto", llm_mode=False):
        rules, err = build_rules(pattern, mode)
        if err:
            return [f"Error: {err}"]
